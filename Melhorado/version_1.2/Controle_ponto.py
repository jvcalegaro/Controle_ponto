...
def dia_existe(arquivo, data_str):
    try:
        with open(arquivo, 'r') as f:
            dados = json.load(f)
        return data_str in dados
    except FileNotFoundError:
        return False	

def editar_dia(arquivo, data_str, novos_turnos):
    pass
...
if dia_existe(CAMINHO_ARQUIVO, DATA_ATUAL):
    input("Esse dia já foi registrado. Deseja editar?")
else:
